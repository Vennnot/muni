...
@export var object_size : Vector2i = Vector2i(16,16)
...
func _set_window(w:Window)->void:
	...
	window.min_size = object_size * Vector2i(zoom)
	window.size = window.min_size
	set_physics_process(true)


func _on_world_scale_changed()->void:
	zoom = Vector2.ONE * ScreenHelper.world_scale


func get_window_pos()->Vector2i: 
	return ScreenHelper.screen_position + (Vector2i(global_position + offset) - object_size / 2) * Vector2i(zoom)
